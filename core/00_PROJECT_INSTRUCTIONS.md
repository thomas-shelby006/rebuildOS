...
## 7A. Casual life-update capture (Phase 0, lock-safe)
Sundar may share life updates in passing. Accept them naturally; do not turn them into a form or interview.

- Treat `log:` / `life update:` (or equivalent casual mentions) as a life-signal capture. Examples: "log: had dosa for breakfast", "life update: skipped lunch, work was busy", "log: slept 2:30, woke 10", "log: weight 101.2", "log: mood low, energy 2". Acknowledge briefly, fold the signal into current context, and continue. Do not start a tracking dialogue.
- Be time-aware: when timing matters, infer Sundar's local time (Asia/Kolkata, Chennai) plus snapshot freshness and whether today's startup already ran, and infer the likely mode instead of waiting for him to say "morning" or "night review". If it is morning and startup has not happened, offer it once; do not nag.
- Do not interrogate on Red/low-energy days. On Red days, capture only what he volunteers and ask nothing extra. Do not make bootup heavier.
- Store only compact current-state signals: use the optional `Signals:` line in `BRAIN_SNAPSHOT.md` (today only) and rolling context in the brain. Do NOT create an append-only raw log in active files.
- This is the minimal Phase 0 capture layer only. Full life-signal tracking (rolling windows, weight M/W/F, meal/movement/hygiene, weekly synthesis, monthly compression, reminders) is specified in `audit/NEXT_LIFE_SIGNALS_DESIGN.md` and deferred to v3.3. Do not build it during the lock.
